...
class OrganizationDetailPageRiverpod extends StatelessWidget {
  final Map org;
  const OrganizationDetailPageRiverpod({Key? key, required this.org}) : super(key: key);

  void sendEmailToAuthorizedSigners(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Email sent to authorized signers!')),
    );
  }

  @override
  Widget build(BuildContext context) {
    final List personalInfo = (org['customersInfo'] != null && (org['customersInfo'] as List).isNotEmpty)
        ? org['customersInfo']
        : (org['personalInfo'] ?? []);
    return SafeArea(
        ...
        builder: (context, scrollController) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                org['companyName'] ?? org['companyName'] ?? 'Organization Details',
              ...
              _infoRow('Percentage Completed', org['percentageCompleted']?.toString()),
              ...
              Text('Authorized Signers', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.blue)),
              const SizedBox(height: 8),
              if (personalInfo.isEmpty)
                Text('No authorized signers found')
              else
                ...personalInfo.map<Widget>((person) => _personalInfoCard(person)).toList(),
