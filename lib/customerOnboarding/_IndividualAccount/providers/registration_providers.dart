...
final networkHandlerProvider =
    Provider<NetworkHandler>((ref) => NetworkHandler());
...
final stepSubmissionProvider =
  ...
  (ref) => StepSubmissionNotifier(
      ref.read(registrationControllerProvider), ref.read(connectivityProvider)),
);
...
class StepSubmissionNotifier extends StateNotifier<StepSubmissionState> {
  ...
  StepSubmissionNotifier(this._controller, this._isOnline)
      : super(StepSubmissionState());

  // Submit first step (Basic Info)
  Future<bool> submitFirstStep(
      RegistrationData data, String phoneNumber, String email) async {
    ...
    try {
      ...
      if (success) {
        ...
        state = state.copyWith(
          ...
          isSuccess: true,
          userId:
              data.customerId, // Use the data parameter that's already passed
        );
        return true;
      } else {
