...
class SimpleFaydaService {
  ...
  Future<void> registerClient() async {
    ...
    await _registrationCompleter!.future; // Removed timeout
  }
  ...
  Future<FaydaUserData> waitForAuthResult() async {
    ...
    _authCompleter = Completer<FaydaUserData>();
    return await _authCompleter!.future; // Removed timeout
  }
      ...
      switch (type) {
        ...
        case 'authentication_result':
          if (data['clientId'] == _clientId &&
            ...
            _authCompleter!.complete(userData);
            close(); // close once we got auth result
          }
          break;
        ...
        case 'authentication_failed':
          if (_registrationCompleter != null && !_registrationCompleter!.isCompleted) {
            _registrationCompleter!.completeError('Server reported registration failure');
          }
          if (_authCompleter != null && !_authCompleter!.isCompleted) {
            _authCompleter!.completeError('Server reported authentication failure');
          }
          close();
          break;
